  ...
  return (
    <div ref={containerRef} className={`relative bg-black overflow-hidden shadow-2xl transition-all ${isFullscreen ? 'fixed inset-0 w-screen h-screen border-0 rounded-none z-50' : 'w-full h-[60vh] sm:h-[70vh] border-4 border-secondary rounded-lg'}`}>
      ...
      <AnimatePresence>{cooldownProgress < 1 && (
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.1, filter: "blur(10px)" }} style={{ top: "calc(50% + 32px)", left: "44%", width: '60px', height: '7.5px' }} className="absolute -translate-x-1/2 bg-black/50 border-2 border-black/55 rounded-full overflow-hidden shadow-[0_0_10px_rgba(0,0,0,0.5)]">
