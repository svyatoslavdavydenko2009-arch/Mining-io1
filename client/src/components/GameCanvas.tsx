...
export function GameCanvas({ user, isFullscreen = false, onFullscreenChange, hitboxEnabled = true, isBackgroundOnly = false }: GameCanvasProps) {
  ...
  const lastSmoothedJoystickDir = useRef<{ dx: number, dy: number } | null>(null);
  ...
  const performMining = (targetX: number, targetY: number) => {
    ...
    const processMiningHit = () => {
      ...
      targets.forEach(t => {
        ...
        const baseScale = t.resource === "wood" ? 1.1 : 0.7;
  ...
  useEffect(() => {
    ...
    const render = () => {
          ...
          if (resType === "stone") {
            ...
            if (rockScale > 1.4) {
              ...
              if (branchSide === 0) {
                ...
                ctx.beginPath(); ctx.moveTo(dsx + 16, dsy + 6); ctx.lineTo(dsx + 16, dsy); ctx.stroke();
      ...
      if (!lastSmoothedJoystickDir.current) {
      ...
      if (isMovingJoystick) {
        ...
        let smoothedDiff = smoothedTargetRotation - smoothBodyRotation.current;
  ...
  return (
    <div ref={containerRef} className={`relative bg-black overflow-hidden shadow-2xl transition-all ${isFullscreen ? 'fixed inset-0 w-screen h-screen border-0 rounded-none z-50' : 'w-full h-[60vh] sm:h-[70vh] border-4 border-secondary rounded-lg'}`}>
      ...
      <AnimatePresence>{cooldownProgress < 1 && (
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.1, filter: "blur(10px)" }} style={{ top: "calc(50% + 32px)", left: "44%", width: '48px', height: '7.5px' }} className="absolute -translate-x-1/2 bg-black/50 border-2 border-black/55 rounded-full overflow-hidden shadow-[0_0_10px_rgba(0,0,0,0.5)]">
      ...
      <div className="absolute top-44 left-4 flex flex-col gap-2 items-start pointer-events-none">
